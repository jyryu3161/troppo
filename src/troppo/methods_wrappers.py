...
class ModelBasedWrapper(object):
    """
    Wrapper for cobamp model readers for the troppo methods.
    ...
    def __init__(self, model: Union[ConstraintBasedModel, AbstractObjectReader], **kwargs):
        self.__model = model
...
class GapfillWrapper(ModelBasedWrapper):
    """Wrapper for gap-fill algorithms."""
...
class ReconstructionWrapper(ModelBasedWrapper):
    """Wrapper for tINIT reconstruction."""

    def run(self, properties: tINITProperties) -> list:
        """
        Run the tINIT reconstruction algorithm.
    ...
    def run_from_omics(self, omics_data: Union[dict, list, tuple, OmicsContainer],
        ...
        kwargs : dict
            Additional keyword arguments for tINITProperties

        Returns
        -------
        dict : Mapping of reaction IDs to boolean (active/inactive)
        """
        ...
        if isinstance(omics_data, OmicsContainer):
            ...
            raise TypeError('omics_data must be an OmicsContainer instance, a dict[str,Number] '
                            'or a tuple/list with reactions')
        ...
        properties = tINIT.properties_class.from_integrated_scores(res, **kwargs)
